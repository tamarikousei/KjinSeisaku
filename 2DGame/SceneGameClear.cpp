...
namespace
{
	constexpr int kMessageFontSize = 40;    // メッセージの文字サイズ（お好みで調整してください）
	constexpr int kMessageThickness = 6;    // フォントの太さ
	constexpr int kMessageY = 500;          // メッセージのY座標。タイトル画像の少し下になるよう仮置きしています
...
{
	m_handle = LoadGraph("date/gameClearBg.png");
...
void SceneGameClear::Draw()
{
	...
	// 表示するメッセージ
	const char* message = "Press SPACE to Title";
...
//	DrawFormatString(300, 240, 0xffffff, "Press SPACE to Title");
	...
	DrawStringToHandle(drawX, kMessageY, message, 0xffffff, m_messageFontHandle);
